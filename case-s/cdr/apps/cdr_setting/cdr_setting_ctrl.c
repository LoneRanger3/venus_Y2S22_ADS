...
lb_int32 interval_record_resp(void *param)
{
	...
	ret = lb_view_get_obj_property_by_id(27, (void *)&property);
	if (0 != ret) {
		printf("[%s]Invalid parameters!\n", __func__);
		return LB_ERROR_NO_MEM;
	}
	ret = lb_view_get_obj_ext_by_id(27, (void *)&obj);
	if (0 != ret) {
		printf("[%s]Invalid parameters!\n", __func__);
		return LB_ERROR_NO_MEM;
	}
...
lb_int32 init_funcs(void)
{
	...
	err |= lb_fmngr_reg_init_func("interval_record_init", interval_record_init);
	err |= lb_fmngr_reg_exit_func("interval_record_exit", interval_record_exit);
...
//	err |= lb_fmngr_unreg_exit_func(watermark_sw_exit);
	...
	err |= lb_fmngr_unreg_init_func(interval_record_init);
	err |= lb_fmngr_unreg_exit_func(interval_record_exit);
...
lb_int32 resp_funcs(void)
{
	...
	err |= lb_reg_resp_msg_func(INTERVAL_RECORD, interval_record_resp);
...
lb_int32 unresp_funcs(void)
{
	...
	err |= lb_unreg_resp_msg_func(interval_record_resp);
