...
void fill_line(uint8_t *buff, int xmin, int xmax, int y, int ymin, int ymax,
	uint32_t line_w, uint32_t line_color, uint8_t en_fill, uint32_t fill_color)
{
	...
	if (line_w > 0) {
		if (y <= line_w + ymin || y >= ymax - line_w) {
			for (xx = xmin; xx <= xmax; xx++) {
				point.x = xx;
				point.y = y;
				adas_draw_set_px(buff, point.x, point.y, 0, line_color);
			}
		} else {
			for (xx = xmin; (xx <= xmin + line_w) && (xx <= xmax); xx++) {
				point.x = xx;
				point.y = y;

				adas_draw_set_px(buff, point.x, point.y, 0, line_color);
			}
			for (xx = xmax; (xx >= xmax - line_w) && (xx >= xmin); xx--) {
				point.x = xx;
				point.y = y;
	...
	if (en_fill) {
		...
		if (end >= start) {
			for (xx = start; xx <= end; xx++) {
				...
				adas_draw_set_px(buff, point.x, point.y, 0, fill_color);
			}
		}
	}
}
 ...
 * @param size edge count of the polygon
 */
void adas_draw_scanfill_polygon(uint8_t *buff, line_point *points, uint8_t size,
	uint32_t line_w, uint32_t line_color, uint8_t en_fill, uint32_t fill_color)
{
	int miny, maxy;
	int sx, sy, tx, ty;
	...
	for (i = miny + 1; i <= maxy - 1; i++) {
		...
		for (i1 = 0, l = size, j1 = l - 1; i1 < l; j1 = i1, i1++) {
			...
			if (ty == sy) {
				if (i == ty) {
					 ...
					 * width, max value minus border width
					 */
					fill_line(buff, xmin, xmax, i, miny, maxy, line_w,
						line_color, en_fill, fill_color);
#if 0
		...
		for (n = 0, m = n + 1; m < temp && index[n] != -1; n += 2, m = n + 1) {
			 ...
			 * width, max value minus border width
			 */
			fill_line(buff, index[n], index[m], i, miny, maxy, line_w,
				line_color, en_fill, fill_color);
#if 0
	...
	uint32_t line_w, uint32_t line_color, uint8_t en_fill, uint32_t fill_color)
{
	...
	adas_draw_polygon(buff, polygon_points, 4, width, line_color);
	adas_draw_scanfill_polygon(buff, polygon_points, 4, line_w, line_color, en_fill,
		fill_color);

	return 0;
}
static int draw_ch(uint8_t *buff, int idx, int x, int y, int w,
	int h, uint8_t rotate, uint32_t color)
{
	int i;
	int j;
	int *color_data;
	...
	for (j = CAR_REC_FONT_HEIGHT - 1; j >= 0; j--) {
		for (i = 0; i < adas_font[idx].width; i++) {
			alpha = adas_font[idx].font_p[j * adas_font[idx].width + i];
			if (alpha != 0x00) {
				if (rotate == SCREEN_NO_ROTATE)
					val = (j + y) * max_w + i + x;
				else if (rotate == SCREEN_ROTATE_90)
					val = (i + y) * max_w + CAR_REC_FONT_HEIGHT - j + x;
				else if (rotate == SCREEN_ROTATE_180)
					val = (CAR_REC_FONT_HEIGHT - j + y) * max_w + x - i;
				else if (rotate == SCREEN_ROTATE_270)
					val = (y - i) * max_w + j + x;

				if (val < 0 || val > ADAS_BKL_LINE_SIZE_MAX - 4)
					continue;

				px = val;
				color_data[px] = alpha << 24 | (color & 0xFFFFFF);
			}
		}
	}
	...
	uint32_t meters, uint32_t color)
{
	...
	snprintf(str, sizeof(str), "%dm", meters);
	len = strlen(str);
	if (len == 0)
		return -2;
	...
	w = abs(area.x2 - area.x1);
	h = abs(area.y2 - area.y1);

	if (rotate == SCREEN_NO_ROTATE) {
		start_x = (w - width_total) / 2;
		start_y = (h - CAR_REC_FONT_HEIGHT) / 2;
	} else if (rotate == SCREEN_ROTATE_90) {
		start_x = (w - CAR_REC_FONT_HEIGHT) / 2;
		start_y = (h - width_total) / 2;
	} else if (rotate == SCREEN_ROTATE_180) {
		start_x = (w - width_total) / 2 + width_total;
		start_y = (h - CAR_REC_FONT_HEIGHT) / 2 + CAR_REC_FONT_HEIGHT;
	} else if (rotate == SCREEN_ROTATE_270) {
		start_x = (w - CAR_REC_FONT_HEIGHT) / 2/* + CAR_REC_FONT_HEIGHT*/;
		start_y = (h - width_total) / 2 + width_total;
	}

	for (i = 0; i < len; i++) {
		for (j = 0; j < sizeof(adas_font)/sizeof(adas_font_t); j++) {
			if (adas_font[j].ch == str[i]) {
				draw_ch(buff, j, start_x, start_y, w,
					h, rotate, color);
				if (rotate == SCREEN_NO_ROTATE)
					start_x += adas_font[j].width + 2;
				else if (rotate == SCREEN_ROTATE_90)
					start_y += adas_font[j].width + 2;
				else if (rotate == SCREEN_ROTATE_180)
					start_x -= adas_font[j].width + 2;
				else if (rotate == SCREEN_ROTATE_270)
					start_y = start_y - adas_font[j].width - 2;
				break;
			}
		}
