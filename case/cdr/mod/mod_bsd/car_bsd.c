/*
 * car_bsd.c - the Auxiliary Driving System of car
 *
...
int update_frame(mm_frame_t *mm_frame)
{
	...
	bsd_get_gps_info(&frame_set_data);
...
static int buf_handle(void *eplayer, al_frame_t *frame)
{
	...
	RT_ASSERT(sem_bsd_mutex);
	rt_sem_take(sem_bsd_mutex, RT_WAITING_FOREVER);
	...
	if (frmid != -1) {
		...
		/* give it to bsd to calculate objects */
		...
		rt_sem_release(sem_bsd_data);
		return ret;
	}

	rt_sem_release(sem_bsd_mutex);
...
int get_buf_handle1()
{
	...
	if (frmid != -1) {
		mm_frame[frmid].multi_yP = frame->info.video.addr[0] +
			(REAR_PREVIEW_WIDTH * REAR_PREVIEW_HEIGHT);
		mm_frame[frmid].multi_uvP = frame->info.video.addr[1] +
			((REAR_PREVIEW_WIDTH * REAR_PREVIEW_HEIGHT) / 2);
		mm_frame[frmid].multi_frame = frame;
 ...
 * Returns -1 if called when get error ; otherwise, return 0
 */
int frame_set_data_init(HDFrameSetBsd *set_data)
{
	int ret = -1;

	memset(set_data, 0x00, sizeof(HDFrameSetBsd));
	ret = bsd_get_time_stamp(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_day_or_night(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_cpuGear(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_vedio_image_info(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_gps_info(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_gsensor_info(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_obd_para(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_warn_sensity(set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_get_run_ped(set_data);
	RT_ASSERT(ret == 0);
...
int init_set_data_init(HDIniSetBsd *init_set_data)
{
	int ret = -1;

	memset(init_set_data, 0x00, sizeof(HDIniSetBsd));

	ret = bsd_module_enable(init_set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_carpara(init_set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_warn_area(init_set_data);
	RT_ASSERT(ret == 0);
	if (check_cali_exist() == 0) {
		/* paremeters should be derived from calibration */
		ret = bsd_set_roi_para(init_set_data);
		RT_ASSERT(ret == 0);
		/* paremeters should be derived from calibration */
		ret = bsd_set_cam_out_para(init_set_data);
		RT_ASSERT(ret == 0);
	} else {
		/* start to calibrate */
		bsd_create_calibration();
		...
		/* finish to calibrate */
		bsd_delete_calibration();
	}


	ret = bsd_set_cam_in_para(init_set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_ped_para(init_set_data);
	RT_ASSERT(ret == 0);
	ret = bsd_set_alarm_sensity(init_set_data);
	RT_ASSERT(ret == 0);
	return ret;
}
...
void *_get_data_proc(void *parameter)
{
	...
	while (1) {
		...
		ret = rt_sem_take(sem_bsd_proc, 100);
		if (-RT_ETIMEOUT == ret) {
		...
		if (frmid != -1) {
			...
			/* bsd draw objects */
			if (bsd_result_cb)
				ret = bsd_result_cb->bsd_result(&frame_get_data);
...
/**
 * bsd_init - initial bsd
 *
 * This function use to initial bsd
 *
 * Returns -1 if called when get error ; otherwise, return 0
 */
int bsd_init(void)
{
	...
	MOD_LOG_D("\n");
	/* fill the two callback function first and run the "create bsd" */
	ret = bsd_lib_init(&init_set_data);
	if (0 != ret) {
		MOD_LOG_E("failed ret:%d\n", ret);
		return ret;
	}
...
int bsd_create(void *recorder_hd)
{
	...
	bsd_get_screen_info(&bsd_screen);
...
{
	car_bsd_status = bsd_status;

	return 0;
}
int car_bsd_get_status(void)
{
	return car_bsd_status;
}
