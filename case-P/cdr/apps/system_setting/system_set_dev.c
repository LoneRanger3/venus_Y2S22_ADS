/*
 * system_set_dev.c - system dev of setting code for LomboTech
 *
...
static dt_t local_dt = {2019, 1, 1, 0, 0, 0};
...
lb_int32 set_date_time(void)
{
	...
	APP_LOG_D("write:year:%d, month:%d, day:%d, hour:%d, minute:%d\n",
...
lb_int32 get_date_time(void)
{
	...
	APP_LOG_D("p_tm->tm_sec:%d, p_tm->tm_min:%d, p_tm->tm_hour:%d\n",
		p_tm->tm_sec, p_tm->tm_min, p_tm->tm_hour);
	APP_LOG_D("p_tm->tm_mday:%d, p_tm->tm_mon:%d, p_tm->tm_year:%d\n",
...
lb_int32 set_language_idx(lb_uint32 idx)
{
	...
	/* set xxx to json */
	cdr_config.system.language_json->valueint = idx;
	cdr_config.system.language_json->valuedouble = idx;
...
lb_int32 get_language_idx(lb_uint32 *idx)
{
	...
	RT_ASSERT(cdr_config.system.language_json != NULL);

	/* get xxx from json */
	*idx = cdr_config.system.language_json->valueint;
...
lb_int32 set_keytone_sw(lb_int8 sw)
{
	...
	RT_ASSERT(cdr_config.system.keytone_enable_json != NULL);

	/* set switch to json */
	cdr_config.system.keytone_enable_json->valueint = sw;
	cdr_config.system.keytone_enable_json->valuedouble = sw;
...
lb_int32 get_keytone_sw(lb_int8 *sw)
{
	...
	*sw = cdr_config.system.keytone_enable_json->valueint;
...
lb_int32 set_fastboot_sw(lb_int32 sw)
{
	...
	/* set switch to json */
	cdr_config.system.fastboot_enable_json->valueint = sw;
	cdr_config.system.fastboot_enable_json->valuedouble = sw;

	rtc_store_en_flag_set(sw);
...
lb_int32 get_fastboot_sw(lb_int32 *sw)
{
	...
	RT_ASSERT(cdr_config.system.fastboot_enable_json != NULL);

	/* get switch from json */
	*sw = cdr_config.system.fastboot_enable_json->valueint;
...
lb_int32 set_rearmirr_sw(lb_int32 sw)
{
	...
	/* set switch to json */
	cdr_config.system.rearmirr_enable_json->valueint = sw;
	cdr_config.system.rearmirr_enable_json->valuedouble = sw;
...
lb_int32 get_rearmirr_sw(lb_int32 *sw)
{
	...
	RT_ASSERT(cdr_config.system.rearmirr_enable_json != NULL);

	/* get switch from json */
	*sw = cdr_config.system.rearmirr_enable_json->valueint;
...
lb_int32 set_accpower_sw(lb_int32 sw)
{
	...
	/* set switch to json */
	cdr_config.system.accpower_enable_json->valueint = sw;
	cdr_config.system.accpower_enable_json->valuedouble = sw;
...
lb_int32 get_accpower_sw(lb_int32 *sw)
{
	...
	RT_ASSERT(cdr_config.system.accpower_enable_json != NULL);

	/* get switch from json */
	*sw = cdr_config.system.accpower_enable_json->valueint;
...
lb_int32 set_standby_value(lb_uint32 value)
{
	...
	RT_ASSERT(cdr_config.system.lcd_standby_time_json != NULL);

	/* set value to json */
	cdr_config.system.lcd_standby_time_json->valueint = value;
	cdr_config.system.lcd_standby_time_json->valuedouble = value;
...
lb_int32 get_standby_value(lb_uint32 *value)
{
	...
	/* get value from json */
	*value = cdr_config.system.lcd_standby_time_json->valueint;
...
static lb_int32 set_volume(lb_uint32 value)
{
	return pcm_set_volume(value, PCM_OUT);
}
/**
 * set_volume_value - set volume to audio and config
 * @param: lb_obj_t object pointer.
 *
 * This function set volume to audio and config
 *
 * Returns 0 if called when get success ; otherwise, return other custom values
 */
lb_int32 set_volume_value(lb_uint32 value)
{
	...
	RT_ASSERT(cdr_config.system.volume_json != NULL);

	/* set value to json */
	cdr_config.system.volume_json->valueint = value;
	cdr_config.system.volume_json->valuedouble = value;
...
/**
 * get_volume_value - get volume from config and set it to audio
 * @param: lb_obj_t object pointer.
 *
 * This function get volume from config and set it to audio
 *
 * Returns 0 if called when get success ; otherwise, return other custom values
 */
lb_int32 get_volume_value(lb_uint32 *value)
{
	...
	/* get value from json */
	*value = cdr_config.system.volume_json->valueint;
...
lb_int32 set_backlight_value(lb_uint32 value)
{
	...
	RT_ASSERT(cdr_config.system.lcd_brightness_json != NULL);

	/* set value to json */
	cdr_config.system.lcd_brightness_json->valueint = value;
	cdr_config.system.lcd_brightness_json->valuedouble = value;
...
/**
 * get_backlight_value - get backlight from config and set it to display
 * @param: lb_obj_t object pointer.
 *
 * This function get backlight from config and set it to display
 *
 * Returns 0 if called when get success ; otherwise, return other custom values
 */
lb_int32 get_backlight_value(lb_uint32 *value)
{
	...
	/* get value from json */
	*value = cdr_config.system.lcd_brightness_json->valueint;
...
lb_int32 restore_factory_setting(void)
{
	...
	if (ret != 0)
		APP_LOG_W("[%s,%d,set volume failed]\n", __FILE__, __LINE__);
	...
	if (ret != 0)
		APP_LOG_W("[%s,%d,update imgbtn failed]\n", __FILE__, __LINE__);
