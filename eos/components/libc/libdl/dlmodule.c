...
void dlmodule_destroy_subthread(struct rt_dlmodule *module, rt_thread_t thread)
{
...
#ifndef ARCH_LOMBO /* cleanup may free thread, which was need below, so move it end */
	...
	/* cleanup after thread struct not be used any more */
	if (thread->cleanup != RT_NULL)
		thread->cleanup(thread);
	...
	/* move cleanup here, FIXME: the below rt_object_delete may crash.. */
	if (thread->cleanup != RT_NULL) {
		LOG_E("fatal err: cleanup may lead crash! thread %s", thread->name);
		thread->cleanup(thread);
	}
...
rt_err_t dlmodule_destroy(struct rt_dlmodule* module)
{
    ...
    /* destory module */
#ifdef ARCH_LOMBO
	size = (u32)SIZE_ALIGN(module->mem_size, PAGE_SIZE);
	mmu_set_attr_space(module->mem_space, virt_to_phys(module->mem_space),
		(u32)module->mem_space + size, SPACE_RWDATA);
	memset((void *)(module->mem_space), 0, size);
...
#ifdef ARCH_LOMBO
            if (module->priority > RT_THREAD_PRIORITY_MAX - 3)
                module->priority = RT_THREAD_PRIORITY_MAX - 3;
            if (module->stack_size < 4096 || module->stack_size > (1024 * 32))
