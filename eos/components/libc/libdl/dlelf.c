...
#ifdef ARCH_LOMBO
	alloc_size = SIZE_ALIGN(module_size, PAGE_SIZE);
	module->mem_space = rt_malloc_align(alloc_size, PAGE_SIZE);
	mmu_set_attr_space(module->mem_space, (u32)virt_to_phys(module->mem_space),
		module->mem_space + alloc_size, SPACE_RWDATA | EXECUTABEL);
#else
...
#ifdef ARCH_LOMBO
	alloc_size = SIZE_ALIGN(module_size, PAGE_SIZE);
	module->mem_space = rt_malloc_align(alloc_size, SZ_4K);
	mmu_set_attr_space(module->mem_space, (u32)virt_to_phys(module->mem_space),
		module->mem_space + alloc_size, SPACE_RWDATA | EXECUTABEL);
#else
    module->mem_space = rt_malloc(module_size);
