...
#ifdef ARCH_LOMBO
	rt_memcpy(csw_out, &csw, sizeof(struct ustorage_csw));
	return RT_EOK;
#else
    return -RT_ERROR;
#endif
    ...
    ustorage_cbw_t cmd, rt_uint8_t* buffer, int timeout)
{
    ...
    do
    {
        ...
        if(cmd->xfer_len != 0)
        {
            ...
            {
#ifdef ARCH_LOMBO
		/* Some SCSI command (like 0x1A) may not receive specific size data
		* and it is legal.
		*/
		if (cmd->dflags == CBWFLAGS_DIR_OUT) {
...
#ifdef ARCH_LOMBO
	/* bugfix: CSW hasn' t been receive if in pipe has been stalled.
	* When pipe has been stalled, clear stall and get csw by function "_pipe_check".
	* So pass pointer of csw is required.
	*/

	/* check in pipes status */
	ret = _pipe_check(intf, stor->pipe_in, &csw);
	if (ret != RT_EOK) {
	...
	ret = _pipe_check(intf, stor->pipe_out, &csw);
	if (ret != RT_EOK) {
...
#ifdef ARCH_LOMBO
rt_err_t rt_usbh_storage_request_mode_sense(struct uhintf *intf,
			rt_uint8_t *buffer)
{
	struct ustorage_cbw cmd;
	int timeout = USB_TIMEOUT_LONG;
	...
	RT_ASSERT(intf->device != RT_NULL);
	RT_DEBUG_LOG(RT_DEBUG_USB, ("rt_usbh_storage_request_mode_sense\n"));
...
rt_err_t rt_usbh_storage_allow_removal(struct uhintf *intf, rt_bool_t allow)
{
	struct ustorage_cbw cmd;
	int timeout = USB_TIMEOUT_LONG;
	...
	RT_ASSERT(intf->device != RT_NULL);
	RT_DEBUG_LOG(RT_DEBUG_USB, ("rt_usbh_storage_allow_removal:%d\n", allow));
