...
rt_err_t rt_udisk_run(struct uhintf* intf)
{
    ...
    rt_err_t ret;
#ifdef ARCH_LOMBO
	/* bugfix: array size is too short for string "ud%d-%d" which
	* cause stack changed
	*/
	char dname[6];
#else
...
//        rt_usbh_clear_feature(intf->device, 0, USB_FEATURE_ENDPOINT_HALT);
	...
	* Some usb 3.0 u-disk are unable to be written after storage reset,
	* power-off will be detected on bable.
	*/
...
#ifdef ARCH_LOMBO
	rt_uint8_t mode_sense[0xc0];
	ret = rt_usbh_storage_request_mode_sense(intf, mode_sense);
	if (ret == -RT_EIO) {
		rt_kprintf("request mode sense failed");
		return ret;
	}

	/* disallow removal, CSW status != 0 is legal */
	ret = rt_usbh_storage_allow_removal(intf, RT_FALSE);
	if (ret != RT_EOK) {
		if (ret == -RT_EIO) {
			rt_kprintf("rt_usbh_storage_allow_removal error\n");
			return ret;
		}
