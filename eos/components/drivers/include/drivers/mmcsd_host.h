...
#ifdef ARCH_LOMBO
#define MMCSD_SUP_ERASE		(1 << 6)	/* Allow erase/trim commands */
#endif
...
#ifdef ARCH_LOMBO
	rt_uint32_t host_index;
	rt_uint32_t max_discard_to;		/* max. discard timeout in ms */
	rt_uint32_t mmcsd_plug_status;
	rt_uint32_t mmcsd_plug_count;
	rt_event_t  mmcsd_plug_event;
#define MMCSD_READY_TO_UMOUNT	(1UL << 0)
#define MMCSD_CARD_PLUG_OUT	(1UL << 1)
#define MMCSD_CARD_PLUG_IN	(1UL << 2)
	rt_thread_t mmcsd_detect_thread;
#endif
