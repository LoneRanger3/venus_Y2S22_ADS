...
#if defined(ARCH_LOMBO) && defined(LOMBO_ISO)
...
struct upipe
{
	...
	rt_uint32_t info;
#ifdef LOMBO_ISO
	int interval;			/* for IOS xfer */
	int error_count;		/* (return) number of ISO errors */
	int number_of_packets;		/* (in) number of ISO packets */
	rt_uint32_t transfer_dma;	/* (in) dma addr for transfer_buffer */
...
rt_uint8_t hcd_get_device_speed(void);
