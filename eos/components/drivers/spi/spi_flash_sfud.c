...
rt_spi_flash_device_t lombo_sfud_flash_probe(const char *flash_name)
{
	...
	err = rt_spi_bus_attach_device(&(qspi->parent), dev_name, bus_name,
		(void *)chip_select);
	if (err) {
		LOG_E("failed to attach %s to %s", dev_name, bus_name);
		rt_free(qspi);
		return RT_NULL;
	}

	return rt_sfud_flash_probe(flash_name, dev_name);
}
...
{
	return (RT_NULL == rtt_dev) ? RT_NULL : (sfud_flash *)rtt_dev->user_data;
}
#endif
...
#ifdef ARCH_LOMBO
	qspi_cfg.parent.mode |= RT_SPI_QSPI;
	if (lombo_sfud_flash_parse_config(spi_flash_dev_name, &qspi_cfg)) {
		rt_kprintf("ERROR: %s failed to parse config!\n", spi_flash_dev_name);
		return RT_NULL;
	}
#endif
...
#ifdef ARCH_LOMBO
		if (4 == qspi_dev->config.qspi_dl_width)
			if (sfud_set_quad_mode(sfud_dev, true)) {
				rt_kprintf("WARNING: %s can not enable Quad I/O mode\n",
					spi_dev_name);
				sfud_qspi_fast_read_enable(sfud_dev, 2);
			}
#endif /* ARCH_LOMBO */
            }
