...
#define ALARMSVC_THREAD_STACK_SIZE	1536
...
static void alarm_update(rt_uint32_t event)
{
	...
	/* delete "&& (alarm != _container.current)" whice in next sentence, */
	/* In order to modify the value of the current alarm during of the alarm. */
...
int rt_alarm_system_init(void)
{
    ...
    tid = rt_thread_create("alarmsvc",
                           ...
                           ALARMSVC_THREAD_STACK_SIZE, 8, 1);
    if (tid != RT_NULL)
