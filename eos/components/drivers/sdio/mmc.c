...
#ifdef ARCH_LOMBO
static void mmc_set_erase_size(struct rt_mmcsd_card *card)
{
...
static int mmc_parse_ext_csd(struct rt_mmcsd_card *card, rt_uint8_t *ext_csd)
{
  if(card == RT_NULL || ext_csd == RT_NULL)
  {
	...
	if (card->ext_csd.rev >= 3) {
		...
		card->ext_csd.hc_erase_size = ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] << 10;
	}
	if (card->ext_csd.rev >= 4) {
		card->ext_csd.sec_trim_mult = ext_csd[EXT_CSD_SEC_TRIM_MULT];
		card->ext_csd.sec_erase_mult = ext_csd[EXT_CSD_SEC_ERASE_MULT];
		card->ext_csd.sec_feature_support = ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT];
		card->ext_csd.trim_timeout = 300 * ext_csd[EXT_CSD_TRIM_MULT];
	}
#endif

  card->flags |=  CARD_FLAG_HIGHSPEED;
#ifdef ARCH_LOMBO
	card->hs_max_data_rate = 50000000;	/* TODO: now we support 50MHz only */
#else
  ...
  card->card_capacity = *((rt_uint32_t *)&ext_csd[EXT_CSD_SEC_CNT]);
	...
	if (card->card_capacity > 4194304)	/* ((2u * 1024 * 1024 * 1024) / 512) */
		card->card_capacity *= 512;
	else
