...
#ifdef ARCH_LOMBO
			if (GET_BITS(resp, 46, 1)) {
				card->erase_size = 1;
			} else if (csd->wr_blk_len >= 9) {
				card->erase_size = GET_BITS(resp, 39, 7) + 1;
...
#ifdef ARCH_LOMBO
int mmcsd_app_sd_status(struct rt_mmcsd_card *card, void *ssr)
{
	int err;
	...
	cmd.arg = 0;
	cmd.flags = RESP_SPI_R2 | RESP_R1 | CMD_ADTC;
	...
	mmcsd_send_request(card->host, &mrq);
...
static int mmcsd_read_ssr(struct rt_mmcsd_card *card)
{
	rt_uint32_t au, es, et, eo;
	int err, i;
	rt_uint32_t *ssr;

	rt_int16_t class_value;
	rt_int16_t speed_class = -1;

	if (!(card->csd.card_cmd_class & CCC_APP_SPEC)) {
		LOG_W("card lacks mandatory SD Status function");
		return 0;
	}
	...
	err = mmcsd_app_sd_status(card, ssr);
	if (err) {
		LOG_W("problem reading SD Status register");
		err = 0;
		goto out;
	}
	 ...
	 */
	au = GET_BITS(ssr, 428 - 384, 4);
	if (au > 0 && au <= 9) {
		card->ssr.au = 1 << (au + 4);
		es = GET_BITS(ssr, 408 - 384, 16);
		et = GET_BITS(ssr, 402 - 384, 6);
		eo = GET_BITS(ssr, 400 - 384, 2);
		if (es && et) {
			card->ssr.erase_timeout = (et * 1000) / es;
			card->ssr.erase_offset = eo * 1000;
		}
	} else {
		LOG_W("SD Status: Invalid Allocation Unit size");
	}

	class_value = GET_BITS(ssr, 440 - 384, 8);
	if (0 <= class_value && class_value <= 3)
		speed_class = 2 * class_value;
	else if (4 == class_value)
		speed_class = 10;
	g_sd_speedclass = speed_class;
	LOG_I("speed_class=%d", g_sd_speedclass);
                                     ...
                                     rt_uint32_t           ocr)
{
	 ...
	 */
	err = mmcsd_read_ssr(card);
	if (err)
		goto err1;

	/* Erase init depends on CSD and SSR */
	mmcsd_init_erase(card);
#endif
