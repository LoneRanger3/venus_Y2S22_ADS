...
#ifdef ARCH_LOMBO
#include <dfs_fs.h>
#include <drivers/block_partition.h>

#include "system/system_mq.h"
#include "power_drv.h"

#define VIRT_SECTOR_SIZE	(512)
...
static rt_err_t rt_mmcsd_req_blk(struct rt_mmcsd_card *card, rt_uint32_t sector,
	void *buf, rt_size_t blks, rt_uint8_t dir)
{
...
#ifdef ARCH_LOMBO
	/* sector size of the block geometry is not 512 any more */
	sector *= (VIRT_SECTOR_SIZE / SECTOR_SIZE);
	blks *= (VIRT_SECTOR_SIZE / SECTOR_SIZE);
#endif
	...
	if (!controller_is_spi(card->host) && dir != 0) {
		do {
			...
			if (err) {
				LOG_D("error %d requesting status", err);
				break;
			}
			/*
	...
	if (cmd.err || data.err || stop.err) {
		LOG_D("mmcsd request blocks error");
		LOG_D("%d,%d,%d, 0x%08x,0x%08x",
...
static rt_err_t rt_mmcsd_control(rt_device_t dev, int cmd, void *args)
{
	...
	case RT_DEVICE_CTRL_BLK_GETGEOME:
		rt_memcpy(args, &blk_dev->geometry,
			sizeof(struct rt_device_blk_geometry));
		break;
#ifdef ARCH_LOMBO
	case RT_DEVICE_CTRL_BLK_ERASE: {
		rt_uint32_t *range = args;
		if (!range) {
			ret = -RT_EINVAL;
			goto out;
		}
		...
		ret = mmcsd_erase(blk_dev->card,
			/* sector size of the block geometry is not 512 any more */
			(part->offset + range[0]) * (VIRT_SECTOR_SIZE / SECTOR_SIZE),
			(range[1] - range[0] + 1) * (VIRT_SECTOR_SIZE / SECTOR_SIZE),
			MMC_ERASE_ARG);
...
out:
	...
	if (ret) {
		if (get_bat_level()) {
			blk_dev->card->has_power_down = 1;
		} else {
			if (blk_dev->card->has_power_down) {
				blk_dev->card->has_power_down = 0;
				mmcsd_change_virt(blk_dev->card->host);
			}
		}
	}
#endif
...
static rt_size_t rt_mmcsd_read(rt_device_t dev, rt_off_t pos, void *buffer,
	rt_size_t size)
{
	rt_err_t err = 0;
	rt_size_t offset = 0;
...
#ifdef ARCH_LOMBO
#if CARD_PERF_MONITOR
	int low_perf = 0;
	int total_ms, start_ms;
#endif
#endif
	...
	while (remain_size) {
		req_size = (remain_size > blk_dev->max_req_size) ?
			blk_dev->max_req_size : remain_size;
		err = rt_mmcsd_req_blk(blk_dev->card,
			part->offset + pos + offset, rd_ptr, req_size, 0);
...
#ifdef ARCH_LOMBO
#if CARD_PERF_MONITOR
	/* performance monitor only care about these request >= 32KByte */
	if ((!err) && (size >= 256)) {
		total_ms = rt_time_get_msec() - start_ms;
		if ((total_ms * 4) >= size) {		/* read performance <= 2MB/s */
			blk_dev->card->low_perf_cnt++;
			blk_dev->card->conti_low_perf_cnt++;
		} else if ((total_ms * 10) <= size) {	/* read performance >= 5MB/s */
			if (blk_dev->card->low_perf_cnt)
				blk_dev->card->low_perf_cnt--;
			blk_dev->card->conti_low_perf_cnt = 0;
		}

		if (blk_dev->card->low_perf_cnt >= 16 &&
					blk_dev->card->low_perf_cnt >= 6) {
			low_perf = 1;
			blk_dev->card->low_perf_cnt = 0;
			blk_dev->card->conti_low_perf_cnt = 0;
		}
#if 0
		LOG_W("performance monitor: %u %u %d", size,
			blk_dev->card->low_perf_cnt, total_ms);
#endif
	}
#endif
#endif
...
#if CARD_PERF_MONITOR
	if (low_perf) {
		LOG_W("low performance card in read size=%u ms=%u",
							size, total_ms);
		lb_system_mq_send(LB_SYSMSG_SD_LOW_PERF, EXT_SDCARD_PATH,
			strlen(EXT_SDCARD_PATH), ASYNC_FLAG);
	}
#endif
#endif
...
static rt_size_t rt_mmcsd_write(rt_device_t dev, rt_off_t pos, const void *buffer,
	rt_size_t size)
{
	rt_err_t err = 0;
	rt_size_t offset = 0;
	...
	while (remain_size) {
		req_size = (remain_size > blk_dev->max_req_size) ?
			blk_dev->max_req_size : remain_size;
		err = rt_mmcsd_req_blk(blk_dev->card, part->offset + pos + offset,
			wr_ptr, req_size, 1);
	...
	if ((!err) && (size >= 256)) {
		total_ms = rt_time_get_msec() - start_ms;
		if ((total_ms * 2) >= size) {		/* write performance <= 1MB/s */
			blk_dev->card->low_perf_cnt++;
			blk_dev->card->conti_low_perf_cnt++;
		} else if ((total_ms * 8) <= size) {	/* write performance >= 4MB/s */
			if (blk_dev->card->low_perf_cnt)
				blk_dev->card->low_perf_cnt--;
			blk_dev->card->conti_low_perf_cnt = 0;
		}
...
#if CARD_PERF_MONITOR
	if (low_perf) {
		LOG_W("low performance card in write size=%u ms=%u",
							size, total_ms);
		lb_system_mq_send(LB_SYSMSG_SD_LOW_PERF, EXT_SDCARD_PATH,
			strlen(EXT_SDCARD_PATH), ASYNC_FLAG);
	}
	...
	if (status == RT_EOK) {
		for (i = 0; i < RT_MMCSD_MAX_PARTITION; i++) {
			...
			if (status == RT_EOK) {
				...
				blk_dev->part.lock = rt_sem_create(sname, 1,
					RT_IPC_FLAG_FIFO);
...
#ifdef ARCH_LOMBO
				blk_dev->geometry.bytes_per_sector = VIRT_SECTOR_SIZE;
				blk_dev->geometry.block_size = card->pref_erase << 9;
				blk_dev->geometry.sector_count = blk_dev->part.size;
				blk_dev->geometry.sector_count /=
					(VIRT_SECTOR_SIZE / SECTOR_SIZE);
				blk_dev->part.offset /=
					(VIRT_SECTOR_SIZE / SECTOR_SIZE);
#else
				...
				if (i == 0) {
					...
					blk_dev->part.size   = 0;
					blk_dev->part.lock = rt_sem_create("sem_sd0",
						1, RT_IPC_FLAG_FIFO);
					...
					rt_device_register(&blk_dev->dev, "sd0",
						...
						RT_DEVICE_FLAG_REMOVABLE |
						RT_DEVICE_FLAG_STANDALONE);
					rt_list_insert_after(&blk_devices,
						&blk_dev->list);

					rt_block_partition_register(&blk_dev->dev);
					break;
				} else {
					rt_free(blk_dev);
					blk_dev = RT_NULL;
		...
		if (sdcard_mount()) {
			lb_system_mq_send(LB_SYSMSG_FS_PART_MOUNT_FAIL,
				EXT_SDCARD_PATH,
				strlen(EXT_SDCARD_PATH),
				ASYNC_FLAG);
			mmcsd_send_partition_mount_event(MMCSD_PART_MOUNT_FAIL);
			LOG_W("sdcard mount failed");
		} else {
			lb_system_mq_send(LB_SYSMSG_FS_PART_MOUNT_OK,
				EXT_SDCARD_PATH,
				strlen(EXT_SDCARD_PATH),
				ASYNC_FLAG);
			mmcsd_send_partition_mount_event(MMCSD_PART_MOUNT_OK);
			LOG_I("sdcard mount ok");
		}
#endif
#endif
	} else {
...
void rt_mmcsd_blk_remove(struct rt_mmcsd_card *card)
{
	...
	lb_system_mq_send(LB_SYSMSG_FS_PART_UNMOUNT,
		...
		strlen(EXT_SDCARD_PATH),
		ASYNC_FLAG);
	mmcsd_send_partition_mount_event(MMCSD_PART_UMOUNT);
	LOG_I("sdcard unmount ok");

	for (l = (&blk_devices)->next, n = l->next;
		l != &blk_devices; l = n, n = l->next) {
		blk_dev = (struct mmcsd_blk_device *)rt_list_entry(l,
			struct mmcsd_blk_device, list);
	...
	for (l = (&blk_devices)->next, n = l->next; l != &blk_devices; l = n) {
		...
		if (blk_dev->card == card) {
			...
			const char *mounted_path =
				dfs_filesystem_get_mounted_path(&(blk_dev->dev));
			if (mounted_path)
