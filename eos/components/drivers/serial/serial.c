 ...
 */
rt_inline rt_size_t
_serial_poll_rx(struct rt_serial_device *serial, rt_uint8_t *data, rt_size_t length)
{
    int ch;
	rt_size_t size;
...
rt_inline rt_size_t
_serial_poll_tx(struct rt_serial_device *serial, const rt_uint8_t *data, rt_size_t length)
{
	rt_size_t size;
    RT_ASSERT(serial != RT_NULL);

	/* lombo: DONOT disable irq during print for audio(cpu mode) performance */
	/* spin_lock_irqsave(&serial->lock, level); */
	...
	/* spin_unlock_irqrestore(&serial->lock, level); */
 ...
 */
rt_inline rt_size_t
_serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data, rt_size_t length)
{
	rt_size_t size;
    struct rt_serial_rx_fifo* rx_fifo;
...
rt_inline rt_size_t
_serial_int_tx(struct rt_serial_device *serial, const rt_uint8_t *data, rt_size_t length)
{
	...
	if (serial->ops->write)
		return serial->ops->write(serial, data, length);
 ...
 */
rt_inline rt_size_t
_serial_dma_rx(struct rt_serial_device *serial, rt_uint8_t *data, rt_size_t length)
{
    rt_base_t level;
...
rt_inline rt_size_t
_serial_dma_tx(struct rt_serial_device *serial, const rt_uint8_t *data, rt_size_t length)
{
#ifdef ARCH_LOMBO
	RT_ASSERT((serial != RT_NULL) && (data != RT_NULL));
	RT_ASSERT(length > 0);

	if (serial->ops->dma_transmit != RT_NULL)
		return serial->ops->dma_transmit(serial,
			(rt_uint8_t *)data, length, RT_SERIAL_DMA_TX);
	else
		return 0;
#else
    rt_base_t level;
...
static rt_err_t rt_serial_close(struct rt_device *dev)
{
    ...
    {
#ifdef ARCH_LOMBO
	port = (struct lombo_uart_port *)serial->parent.user_data;
	RT_ASSERT(port != RT_NULL);

	if (port->dmachn)
		dma_release_channel(port->dmachn);
#else
        struct rt_serial_tx_dma* tx_dma;
