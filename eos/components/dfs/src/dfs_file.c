...
#include "msh.h"
int cmd_cp_root_to(int argc, char **argv)
{
	struct dfs_fd root_fd;
	...
	if (argc == 2) {
		...
		if ((pathname == NULL) || (pathname[0] != '/')) {
			rt_kprintf("Usage: cp_root_to path\n");
			return 0;
		}
		if ((pathname[0] == '/') && (pathname[1] == '\0')) {
			rt_kprintf("cp_root_to self\n");
			return 0;
		}
		...
		if (dfs_file_open(&root_fd, path, O_DIRECTORY) == 0) {
			rt_kprintf("cp_root_to %s start ...\n", pathname);
			do {
				memset(&root_dirent, 0, sizeof(struct dirent));

				dfs_file_getdents(&root_fd, &root_dirent,
						sizeof(struct dirent));
				if (root_dirent.d_name[0] != '\0') {
					/* build full path for each file */
					fullpath = dfs_normalize_path(path,
							root_dirent.d_name);
					if (fullpath == NULL) {
						rt_kprintf("inv fullpath\n");
						break;
					}

					if (rt_strncmp(fullpath, pathname,
							rt_strlen(fullpath)))
						copy(fullpath, pathname);

					rt_free(fullpath);
				}
			} while (root_dirent.d_name[0] != '\0');
			rt_kprintf("cp_root_to %s finish\n", pathname);
			dfs_file_close(&root_fd);
		} else
			rt_kprintf("Open %s directory fail\n", path);
	} else
		rt_kprintf("Usage: cp_root_to path\n");

	return 0;
}
FINSH_FUNCTION_EXPORT_ALIAS(cmd_cp_root_to, __cmd_cp_root_to, cp root to dir);
#endif
#endif /* ARCH_LOMBO */
