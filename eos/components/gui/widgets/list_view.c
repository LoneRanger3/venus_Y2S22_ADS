...
rtgui_type_t *rtgui_listview_type_get(void)
{
	static rtgui_type_t *listview_type = RT_NULL;

	if (!listview_type)
	{
		listview_type = rtgui_type_create("listview", RTGUI_CONTAINER_TYPE,
			sizeof(struct rtgui_list_view),
			RTGUI_CONSTRUCTOR(_rtgui_list_view_constructor),
			RTGUI_DESTRUCTOR(NULL));
	}

	return listview_type;
}
...
static void rtgui_list_view_onlistdraw(struct rtgui_list_view* view, struct rtgui_dc *dc)
{
	...
	{
		if (page_index + index >= view->items_count)
			break;
