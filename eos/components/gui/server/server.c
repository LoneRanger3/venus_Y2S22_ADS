...
{
	struct rtgui_panel* panel = NULL;

	panel = rtgui_panel_find(event->panel_name);
	if (panel != RT_NULL)
	{
...
void rtgui_server_destroy_application(struct rtgui_event_panel_detach* event)
{
	...
	if (panel != RT_NULL)
	{
...

		{
			...
			if (tid != RT_NULL) /* lomboswer, to check here */
			{
...
void rtgui_server_handle_mouse_btn(struct rtgui_event_mouse* event)
{
	...
	rt_thread_t tid_temp; /* lomboswer */
...
#if 1 /* lomboswer, must check here */
		tid_temp = rtgui_panel_get_active_thread(panel);
		if (tid_temp != RT_NULL)
		{
			/* send mouse event to thread */
