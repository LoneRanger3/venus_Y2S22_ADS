...
/*
 * cpu status definations
 */
#define CPU_STATUS_ONLINE	0	/* CPU is normal running */
#define CPU_STATUS_OFFLINE	1	/* CPU is in power down state */
#define CPU_STATUS_DYING	2	/* CPU is being killed, soon power down */
#define CPU_STATUS_UPING	3	/* CPU is powering up */

/*
 * events for daemon thread
 */
#define DAEMON_EVT_CPU_DYING	BIT(0)	/* CPU is dying */
#define DAEMON_EVT_THD_DEFUNCT	BIT(1)	/* these is thread defuncted, need release res */
#define DAEMON_EVT_PM_CHANGE	BIT(2)	/* pm state change, need to enter new state */

/*
 * events for guard thread
 */
#define GUARD_EVT_PRINT		BIT(0)	/* the event to let guard thread print log_buf */

/*
 * cpu time statistics
 */
enum {
	TIME_STATS_TOTAL = 0,	/* total time from cpu scheduler start */
	TIME_STATS_IDLE,	/* total time that cpu in idle/wfi */
	TIME_STATS_START,	/* the start timestamp, to calc total time */
#ifdef SCHED_LOCK_DEBUG
	TIME_STATS_SCHED_LOCK,	/* total time that cpu wait scheduler lock */
#endif
#ifdef SPIN_LOCK_DEBUG
	TIME_STATS_SPIN_LOCK,	/* total time that cpu wait spin lock */
#endif
	TIME_STATS_CNT,
};
#endif /* ARCH_LOMBO */
...
#ifdef ARCH_LOMBO
#define RT_IPI_INV_TLB			1 /* invalidate tlb */
#define RT_IPI_INV_ICACHE		2 /* invalidate icache */
#define RT_IPI_WAIT			3 /* hung other cores */
#ifdef RT_USING_BIND_TIMER
#define RT_IPI_DISABLE_LOCAL_INT		4 /* stop a local timer */
#define RT_IPI_ENABLE_LOCAL_INT			5 /* start a local timer */
#endif
#ifdef RT_USING_DEADLOCK_MONITOR
/* dump target cpu's current thread stack when deadlock occur */
#define RT_IPI_DUMP_CURTHREAD_STACK	6
#endif
#endif /* ARCH_LOMBO */
...
#endif
#ifdef ARCH_LOMBO
	int status; /* cpu status, should be accessed with _cpus_lock locked */
	struct rt_completion die;	/* sync for cpu up/down */
	struct rt_event *ev_dam;	/* to let daemon thread run */

#ifdef CPU_STATS
	/* cpu statistics */
	rt_uint64_t time_stat[TIME_STATS_CNT];	/* unit: 1/24usec (one tick of osc24m) */

	float loading;             /* cpu current loading. [0, 100] */
	float spin_lock_loading;   /* loading caused by waiting spin lock. [0, 100] */
	float sched_lock_loading;  /* loading caused by waiting sched lock. [0, 100] */
#endif
#endif /* ARCH_LOMBO */
...
#if defined(ARCH_LOMBO) && defined(RT_USING_SMP) && defined(THREAD_STATS)
	/*
	 * time unit for below is 1/24 us (one tick of osc24m)
	 */
	rt_uint64_t total_running;	/* total running time */
	rt_uint64_t last_run_start;	/* last running start timestamp */
#ifdef SCHED_LOCK_DEBUG
	rt_uint64_t total_sched_lock;	/* total time that busy waiting sched lock */
#endif
#ifdef SPIN_LOCK_DEBUG
	rt_uint64_t total_spin_lock;	/* total time that busy waiting spin lock */
#endif
	/*
	 * temp var to calc thread's cpu loading, unit ms
	 */
	rt_uint32_t tmp_total_period; /* total running time for a period, eg: 2s */
	rt_uint32_t tmp_last_stamp;   /* timestamp of total_running, for calc */
	float loading;                /* thread loading, range [0, 100] */

#ifdef SCHED_LOCK_DEBUG
	rt_uint32_t tmp_sched_lock_period; /* total waiting time (in us) for a period */
	rt_uint32_t tmp_sched_lock_last;   /* timestamp of total_sched_lock(in us) */
	float sched_lock_loading;   /* loading caused by waiting sched lock. [0, 100] */
#endif
#ifdef SPIN_LOCK_DEBUG
	rt_uint32_t tmp_spin_lock_period; /* total waiting time (in us) for a period */
	rt_uint32_t tmp_spin_lock_last;   /* timestamp of total_spin_lock(in us) */
	float spin_lock_loading;    /* loading caused by waiting spin lock. [0, 100] */
#endif

#endif /* ARCH_LOMBO && RT_USING_SMP && THREAD_STATS */
};
...
/* #if defined(ARCH_LOMBO) && defined(RT_USING_HOOK) && defined(RT_DEBUG) */
	...
	char owner[RT_NAME_MAX];			/* owner thread of the object */
#endif
