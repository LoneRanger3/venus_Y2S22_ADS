...
static void _game_over(void)
{
	...
	rtgui_view_show((rtgui_view_t*)g_app_info.function_view, RT_FALSE);
}
...
static rt_bool_t workbench_event_handler(rtgui_widget_t *widget, rtgui_event_t *event)
{
	if (event->type == RTGUI_EVENT_KBD)
	{
		...
		if (((ekbd->type == RTGUI_KEYUP) && ekbd->key == RTGUIK_HOME)
				&& !(g_app_info.workbench->flag & RTGUI_WORKBENCH_FLAG_MODAL_MODE))
		{
			/* active home view */
...
static void listitem_action_return(struct rtgui_widget* widget, void* parameter)
{
	...
	g_app_info.workbench->flag |= RTGUI_WORKBENCH_FLAG_CLOSED; /* let rtgui_workbench_event_loop exit, lomboswer */
}

static void listitem_action_start(struct rtgui_widget* widget, void* parameter)
{
	...
	/* create timer */
	g_app_info._timer = rtgui_timer_create(100, RT_TIMER_FLAG_PERIODIC, _timer_timeout, RT_NULL);
...
static void listitem_action_continue(struct rtgui_widget* widget, void* parameter)
{
	rt_kprintf("%s %d!\n", __func__, __LINE__);
}

static void listitem_action_adjust(struct rtgui_widget* widget, void* parameter)
{
	rt_kprintf("%s %d!\n", __func__, __LINE__);
}

static void listitem_action_description(struct rtgui_widget* widget, void* parameter)
{
	rt_kprintf("%s %d!\n", __func__, __LINE__);
}
...
void tetris_main(void)
{
	...
	{
		rt_kprintf("%s %d! can't find panel 'main'\n", __func__, __LINE__);
		rt_mq_delete(mq);
		return;
	}
