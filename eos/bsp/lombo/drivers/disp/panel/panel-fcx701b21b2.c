/*
 * panel-fcx701b21b2.c - Panel manufacturer module driver code for LomboTech
 *
...
#define DBG_SECTION_NAME	"FCX701B21B2"
 ...
 */
#define DISP_PANEL_MODULE	"panel_fcx701"
...
/* vo dev config */
static void fcx701b21b2_dev_config(vo_device_t *vo_dev)
{
...
/* Power up */
static void fcx701b21b2_dev_init(vo_device_t *vo_dev)
{
	/* regulator init */
	...
	/* vref init */
	panel_vref_io_init(DISP_PANEL_MODULE, DISP_VREF_GROUP);
	panel_io_vref_set(DISP_PANEL_MODULE, IO_HIGH);
...
/* Power down */
static void fcx701b21b2_dev_exit(vo_device_t *vo_dev)
{
	/* back light pwm exit */
	...
	/* vref exit */
	panel_io_vref_set(DISP_PANEL_MODULE, IO_LOW);
	panel_vref_io_exit(DISP_PANEL_MODULE, DISP_VREF_GROUP);
...
static int fcx701b21b2_dev_set_backlight_state(bool state)
{
	int ret;
	...
	if (ret == RT_ERROR)
		ret = panel_set_backlight_state(state);

	return ret;
}

static int fcx701b21b2_dev_set_backlight_value(u32 value)
{
	return panel_set_backlight_value(DISP_PANEL_MODULE, value);
}

static int rt_fcx701b21b2_panel_init(void)
{
	/* this memory not need to free  */
	disp_panel_ctl_t *pctl = rt_malloc(sizeof(disp_panel_ctl_t));

	pctl->ops.dev_config = fcx701b21b2_dev_config;
	pctl->ops.dev_init = fcx701b21b2_dev_init;
	pctl->ops.dev_exit = fcx701b21b2_dev_exit;
	pctl->ops.dev_set_backlight_state = fcx701b21b2_dev_set_backlight_state;
	pctl->ops.dev_set_backlight_value = fcx701b21b2_dev_set_backlight_value;
...
INIT_APP_EXPORT(rt_fcx701b21b2_panel_init);
