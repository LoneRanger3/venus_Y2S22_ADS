/*
 * audio_core.h - audio core header for LomboTech
 * audio codec driver interface and define
 *
...
#define RECORD_MP_COUNT                     4
#define RECORD_MP_BLOCK_SIZE                4096

#define REPLAY_MP_COUNT                     4
#define REPLAY_MP_BLOCK_SIZE                4096
...
struct rt_audio_ops {
	rt_err_t  (*getcaps)     (struct rt_audio_device *audio,
						struct rt_audio_caps *caps);
	rt_err_t  (*configure)   (struct rt_audio_device *audio,
						struct rt_audio_caps *caps);
	...
	rt_size_t   (*transmit)  (struct rt_audio_device *audio,
					const void *writeBuf,
					void *readBuf, rt_size_t size);
	/* get page size of codec or private buffer's info */
	void    (*buffer_info)   (struct rt_audio_device *audio,
...
struct rt_audio_replay {
	...
	struct rt_audio_frame cur_frame;
	rt_uint32_t cur_frame_offset;
	rt_uint32_t buf_left;
};
...
struct rt_audio_device {
	...
	struct audio_mp            *tx_mp;
	struct audio_mp            *rx_mp;

	struct rt_audio_replay     *replay;
	struct rt_audio_record     *record;
	struct rt_audio_configure  p_config;
	struct rt_audio_configure  r_config;
	spinlock_t lock;
};
...
void        rt_audio_tx_done(struct rt_audio_device *audio,
					rt_uint8_t *pbuf, rt_size_t len);
